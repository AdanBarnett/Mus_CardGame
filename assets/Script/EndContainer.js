...
export default cc.Class({
  ...
  setValues(
    ...
    coins_history[0][0].play.forEach((play) => {
      if (play.coin > 0) {
        let sg = cc.instantiate(this.score_group);
        const sgc = sg.getComponent("ScoreGroup");
    ...
    coins_history[0][1].play.forEach((play) => {
      if (play.coin > 0) {
        let sg = cc.instantiate(this.score_group);
        const sgc = sg.getComponent("ScoreGroup");
        sgc.setValues(play.coin, play.type);
    ...
    coins_history[1][0].play.forEach((play) => {
      if (play.coin > 0) {
        let sg = cc.instantiate(this.score_group);
        const sgc = sg.getComponent("ScoreGroup");
        sgc.setValues(play.coin, play.type);
    ...
    coins_history[1][1].play.forEach((play) => {
      if (play.coin > 0) {
        let sg = cc.instantiate(this.score_group);
        const sgc = sg.getComponent("ScoreGroup");
        sgc.setValues(play.coin, play.type);
    ...
    // pairs
    let t1pp = coins_history[2][0].instant;
    let t2pp = coins_history[2][1].instant;
    let t1pa = coins_history[2][0].end;
    let t2pa = coins_history[2][1].end;
    if (t1pp > 0) {
      let sg = cc.instantiate(this.score_group);
      const sgc = sg.getComponent("ScoreGroup");
      sgc.setValues(t1pp, "(Pass envido)");
      this.team1_pairs.addChild(sg);
    }
    if (t2pp > 0) {
      let sg = cc.instantiate(this.score_group);
      const sgc = sg.getComponent("ScoreGroup");
      sgc.setValues(t2pp, "(Pass envido)");
      this.team2_pairs.addChild(sg);
    }
    if (t1pa > 0) {
      let sg = cc.instantiate(this.score_group);
      const sgc = sg.getComponent("ScoreGroup");
      sgc.setValues(t1pa, "(Win envido)");
      this.team1_pairs.addChild(sg);
    }
    if (t2pa > 0) {
      let sg = cc.instantiate(this.score_group);
      const sgc = sg.getComponent("ScoreGroup");
      sgc.setValues(t2pa, "(Win envido)");
      this.team2_pairs.addChild(sg);
    }
    ...
    if (g1 > 0 || g2 > 0) {
      let t1gp = coins_history[3][0].instant;
      let t2gp = coins_history[3][1].instant;
      let t1ga = coins_history[3][0].end;
      let t2ga = coins_history[3][1].end;
      if (t1gp > 0) {
        let sg = cc.instantiate(this.score_group);
        const sgc = sg.getComponent("ScoreGroup");
        sgc.setValues(t1gp, "(Pass envido)");
        this.team1_game.addChild(sg);
      }
      if (t2gp > 0) {
        let sg = cc.instantiate(this.score_group);
        const sgc = sg.getComponent("ScoreGroup");
        sgc.setValues(t2gp, "(Pass envido)");
        this.team2_game.addChild(sg);
      }
      if (t1ga > 0) {
        let sg = cc.instantiate(this.score_group);
        const sgc = sg.getComponent("ScoreGroup");
        sgc.setValues(t1ga, "(Win envido)");
        this.team1_game.addChild(sg);
      }
      if (t2ga > 0) {
        let sg = cc.instantiate(this.score_group);
        const sgc = sg.getComponent("ScoreGroup");
        sgc.setValues(t2ga, "(Win envido)");
        this.team2_game.addChild(sg);
      }
      ...
      let t1gp1 = coins_history[4][0].instant;
      let t2gp1 = coins_history[4][1].instant;
      let t1ga1 = coins_history[4][0].end;
      let t2ga1 = coins_history[4][1].end;
      if (t1gp1 > 0) {
        if (t1gp1 === 1) {
          let sg = cc.instantiate(this.score_group);
          const sgc = sg.getComponent("ScoreGroup");
          sgc.setValues(2, "(Point and Fear)");
          this.team1_game.addChild(sg);
        }
        if (t1gp1 > 1) {
          let sg = cc.instantiate(this.score_group);
          const sgc = sg.getComponent("ScoreGroup");
          sgc.setValues(t1gp1, "(Pass envido)");
          this.team1_game.addChild(sg);
        }
      }
      if (t2gp1 > 0) {
        ...
        if (t2gp1 > 1) {
          let sg = cc.instantiate(this.score_group);
          const sgc = sg.getComponent("ScoreGroup");
          sgc.setValues(t2gp1, "(Pass envido)");
          this.team2_game.addChild(sg);
        }
      }
      if (t1ga1 > 0) {
        let sg = cc.instantiate(this.score_group);
        const sgc = sg.getComponent("ScoreGroup");
        sgc.setValues(t1ga1, "(Win envido)");
        this.team1_game.addChild(sg);
      }
      if (t2ga1 > 0) {
        let sg = cc.instantiate(this.score_group);
        const sgc = sg.getComponent("ScoreGroup");
        sgc.setValues(t2ga1, "(Win envido)");
        this.team2_game.addChild(sg);
      }
      if (t1gp1 !== 1 && t2gp1 !== 1) {
  ...
  onBigClickButton() {
    ...
    if (this._winCards[0].user === -1) return;
  ...
  onSmallClickButton() {
    ...
    this.selected.active = true;
    if (this._winCards[1].user === -1) return;
  ...
  onPairsClickButton() {
    ...
    this.selected.active = true;
    if (this._winCards[2].user === -1) return;
  ...
  onGameClickButton() {
    ...
    this.selected.active = true;
    if (this._winCards[3].user === -1) return;
